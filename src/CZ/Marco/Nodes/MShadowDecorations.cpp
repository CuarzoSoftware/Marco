...
void MShadowDecorations::onBind() noexcept
{
    ...
    // The shadow must match the surface's rounded corners; rebuild it when they change.
    surface()->onBorderRadiusChanged.subscribe(this, [this](){ m_shadow.rebuild(); });
...
void MShadowDecorations::updateInvisibleRegion() noexcept
{
    ...
    // shadow is not composited there.
    const SkIRect central { SkIRect::MakeLTRB(m.fLeft, m.fTop, w - m.fRight, h - m.fBottom) };

    SkRegion invisible;

    if (!central.isEmpty())
    {
        invisible.setRect(central);
        invisible.op(SkIRect::MakeXYWH(central.fLeft,        central.fTop,          rTL, rTL), SkRegion::kDifference_Op);
        invisible.op(SkIRect::MakeXYWH(central.fRight - rTR, central.fTop,          rTR, rTR), SkRegion::kDifference_Op);
        invisible.op(SkIRect::MakeXYWH(central.fRight - rBR, central.fBottom - rBR, rBR, rBR), SkRegion::kDifference_Op);
        invisible.op(SkIRect::MakeXYWH(central.fLeft,        central.fBottom - rBL, rBL, rBL), SkRegion::kDifference_Op);
    }
...
void MShadowDecorations::ShadowNode::rebuild() noexcept
{
    ...
    m_image = app->theme()->csdShadow(scale(), inner, m_deco->m_radius, m_deco->m_offset.x(), m_deco->m_offset.y(), m_deco->surface()->borderRadius(), m_clampSides);
...
void MShadowDecorations::ShadowNode::renderEvent(const AKRenderEvent &e)
{
    ...
    // Skip the interior fully covered by the opaque content (perf only: the image centre is
    // transparent, so painting there is harmless but wasteful). Insetting by the largest corner
    // keeps the shadow's rounded corners intact.
    SkRegion maskedDamage { e.damage };
    SkIRect center { SkIRect::MakeLTRB(m.fLeft, m.fTop, w - m.fRight, h - m.fBottom) };
    center.inset(std::max({ maxL, maxR, maxT, maxB }) + 1, std::max({ maxL, maxR, maxT, maxB }) + 1);
    if (!center.isEmpty())
        maskedDamage.op(center, SkRegion::Op::kDifference_Op);
    ...
    // Clamped: 9-slice the minimal image. Corner extents cover the shadow margin, the side's largest
    // rounded corner and the blur transition (~radius) so the stretched 1px middle strip is sampled
    // from the fully-developed straight edge (matching MTheme::csdShadow's minClamp).
    const Int32 cLW { m.fLeft   + maxL + rad };
    const Int32 cRW { m.fRight  + maxR + rad };
    const Int32 cTH { m.fTop    + maxT + rad };
    const Int32 cBH { m.fBottom + maxB + rad };
