...
void MPopup::Imp::xdg_popup_done(void *data, xdg_popup */*xdgPopup*/)
{
    ...
    CZCore::Get()->sendEvent(CZCloseEvent(), *popup);
...
xdg_positioner *MPopup::Imp::createPositioner() noexcept
{
    ...
    xdg_positioner *positioner { xdg_wm_base_create_positioner(MApp::Get()->wl.xdgWmBase) };
    ...
    if (MApp::Get()->wl.xdgWmBase.version() >= 3)
    {
...
void MPopup::Imp::create() noexcept
{
    ...
    xdgSurface = xdg_wm_base_get_xdg_surface(MApp::Get()->wl.xdgWmBase, obj.wlSurface());
    ...
    if (grab)
    {
        xdg_popup_grab(xdgPopup, MApp::Get()->wl.seat, grab->serial);
        grab.reset();
    }
