...
public:
    ...
    void startResize() noexcept
    {
        ...
        resizeStartWidth = worldRect().width();
        resizeStartPointerX = AKApp::Get()->pointer().pos().x();
        resizing = true;
    }
    ...
    void updateResize() noexcept
    {
        if (!resizing) return;
        layout().setWidth(
            std::max(0.f, std::min(resizeStartWidth + AKApp::Get()->pointer().pos().x() - resizeStartPointerX, layout().maxWidth().value)));
    }
        ...
        AKSolidColor(0xFFFFFFFF, parent)
    {
        ...
        */
        ...
        for (auto &iconName : iconNames)
        {
            ...
            auto image { RImage::LoadFile(xdgIcon->getPath(XDGIcon::SVG), *fmt, {256, 256}, &cons) };
...
int main()
{
    ...
    app->onScreenUnplugged.subscribe(app.get(), [](MScreen &screen){
